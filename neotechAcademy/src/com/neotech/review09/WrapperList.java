...
public class WrapperList {
	public static void main(String [] args) {
		...
		System.out.println("Does the lsit contain the number 8 -> "+contains);
		...
		System.out.println("Hıw can we iterate the ArrayList?");
		
		System.out.println("-> 1st way, using normal for loop");
		
		for(int i=0; i<numbers.size(); i++) {
			Integer element=numbers.get(i); //auto-unboxing -> you are getting an integer and  
			// store it in a primitive data type
		...
		// it.hasNext(); //return true false if there is a next element
		// it.next(); //will return the next element from the list
		// it.remove(); //will remove the current element from the list
